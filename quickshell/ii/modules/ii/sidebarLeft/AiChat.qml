...
Item {
    ...
    property var allCommands: [
        ...
        {
            name: "mode",
            description: Translation.tr("Switch AI mode between Chat and Agent."),
            execute: args => {
        ...
        {
            name: "chat",
            description: Translation.tr("Switch to Chat mode."),
            execute: () => {
                Ai.setMode("chat");
            }
        },
        {
            name: "agent",
            description: Translation.tr("Switch to Agent mode."),
            execute: () => {
                Ai.setMode("agent");
            }
        },
        ...
        Item {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    StatusItem {
                        icon: Ai.currentModelRequiresApiKey ? (Ai.currentModelHasApiKey ? "key" : "key_off") : "verified_user"
                        statusText: ""
                        description: Ai.currentModelRequiresApiKey ? (Ai.currentModelHasApiKey ? Translation.tr("API key is set\nChange with /key YOUR_API_KEY") : Translation.tr("No API key\nSet it with /key YOUR_API_KEY")) : Translation.tr("%1 does not require an API key").arg(Ai.getModel().name)
                    }
                    StatusSeparator {}
                    StatusItem {
            ...
            RowLayout { // Input field and send button
                ...
                ScrollView {
                    ...
                    StyledTextArea { // The actual TextArea (inside ScrollView to enable scrolling)
                        ...
                        placeholderText: Translation.tr('Message the model... "/" for commands')
                        ...
                        onTextChanged: {
                            ...
                            if (messageInputField.text.length === 0) {
                                ...
                                root.suggestionQuery = messageInputField.text.split(" ")[1] ?? "";
                                const modeResults = Fuzzy.go(root.suggestionQuery, Ai.modeList.map(mode => {
                                    return {
                                ...
                                root.suggestionList = modeResults.map(mode => {
                                    return {
                                        name: `${messageInputField.text.trim().split(" ").length == 1 ? (root.commandPrefix + "mode ") : ""}${mode.target}`,
                                        displayName: `${Ai.modeDisplayName(mode.target)}`,
